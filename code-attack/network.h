      ...
      for (wave = 1; wave <= SM_Parameters->num_waves; wave++)
      {
           ...
           )
         {
             if( (NTW->Phase == 2) || (NTW->Phase == 3) || (NTW->Phase == 5) || (NTW->Phase == 6))
             {
                ...
                if(NTW->dw == 666)
                   NTW->SendPK[NTW->Phase][SM_Parameters->num_waves - 2][NTW->ThisLoad]++;
                else
          		   NTW->SendPK[NTW->Phase][NTW->dw - 2][NTW->ThisLoad]++;
         ...
         /* Buffer assigment  */
         {
            ...
            if (NTW->OutBuffer[Pos_buffer][wave][buffer+1] == 1)
            {
               ...
               if( (NTW->Bsource[Pos_buffer][wave][buffer] != VirtualNode) && (dest != ndr) )
               {
                  ...
                  {
                      if(NTW->dw == 666)
                         NTW->LostPK[NTW->Phase][SM_Parameters->num_waves - 2][NTW->ThisLoad]++;
                      else
...
void Absorption (int ndr,int link,int Olink,int wave,int dest,int LoB, struct NETWORK *NTW, struct SPARAMETERS *SM_Parameters)
{
   ...
   if (LoB == 1)
   {
	  ...
	  {
          if(NTW->dw == 666)
             NTW->AbsPK[NTW->Phase][SM_Parameters->num_waves - 2][NTW->ThisLoad]++;
          else
    		 NTW->AbsPK[NTW->Phase][NTW->dw - 2][NTW->ThisLoad]++;
	  }
	  else
	  {
...
//		 ( (fiber >= NTW->nbr_of_nodes) && (fiber < NTW->n_links)))
...

   {
      ...
      // Failure_Waves [TINI] [TFIN] [source] [target] [fiber] [wave]
      //                 0      1       2        3        4      5
      /*if(blocked == FREE)
         for(i = 0; i < SM_Parameters->Operation_Time; i++)
         {
            if(NTW->Failure_Waves[i][0] == 0)
               break;

            if( (NTW->tempo >= NTW->Failure_Waves[i][0]) && // lower limit
              (NTW->tempo <= NTW->Failure_Waves[i][1]) )  // upper limit
            {
               if(   (NTW->Failure_Waves[i][7] != dest) && // Different dest AND
                     (NTW->Failure_Waves[i][5] >= wave) &&   // SAME wavelengths
                     (NTW->Failure_Waves[i][4] == INITIAL_FIBER) // ON this link
                  )
               {
                  Wave_Status = 1;
                  break;
